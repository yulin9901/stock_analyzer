...
"""
    ...
    if openai_api_key == "YOUR_OPENAI_API_KEY_HERE" or not openai_api_key:
        ...
        try:
            response = requests.post(sealos_api_url, headers=headers, json=payload, timeout=60)
            ...
            # 解析AI响应
            strategies = parse_ai_response(raw_ai_response, trading_pairs, daily_summary_id, ai_result)
...
def parse_ai_response(
    ...
    Returns:
        List[Dict[str, Any]]: 解析后的交易策略列表
    """
    strategies = []

    # 将原始响应分割为每个交易对的部分
    sections = raw_response.split("1. 交易对:")

    if len(sections) <= 1:
        logger.warning("AI响应格式不符合预期，尝试其他分割方法")
        sections = raw_response.split("交易对:")
    ...
    for section in sections[1:]:
        try:
            ...
            # 解析交易对
            trading_pair_line = lines[0].strip()
            for pair in trading_pairs:
            ...
            for line in lines:
                ...
                if "仓位类型:" in line or "Position:" in line:
                    position_type = line.split(":", 1)[1].strip().upper()
                ...
                elif "入场价格:" in line or "Entry Price:" in line:
                    try:
                ...
                elif "止损价格:" in line or "Stop Loss:" in line:
                    try:
                        price_str = line.split(":", 1)[1].strip()
                        price_str = ''.join(c for c in price_str if c.isdigit() or c == '.' or c == ',')
                        price_str = price_str.replace(',', '')
                ...
                elif "止盈价格:" in line or "Take Profit:" in line:
                    try:
                        price_str = line.split(":", 1)[1].strip()
                        price_str = ''.join(c for c in price_str if c.isdigit() or c == '.' or c == ',')
                        price_str = price_str.replace(',', '')
                ...
                elif "仓位大小:" in line or "Position Size:" in line:
                    try:
                        size_str = line.split(":", 1)[1].strip()
                        size_str = ''.join(c for c in size_str if c.isdigit() or c == '.' or c == '%')
                        size_str = size_str.replace('%', '')
                        strategy["position_size_percentage"] = float(size_str)
                    except (ValueError, IndexError):
                        logger.warning(f"无法解析仓位大小: {line}")

                elif "杠杆倍数:" in line or "Leverage:" in line:
                    try:
                        leverage_str = line.split(":", 1)[1].strip()
                        leverage_str = ''.join(c for c in leverage_str if c.isdigit() or c == '.' or c == 'x' or c == 'X')
                        leverage_str = leverage_str.replace('x', '').replace('X', '')
                    ...
                    # 收集理由的所有行
                    for i in range(reasoning_index, len(lines)):
                        reasoning_part = lines[i]
                        if i == reasoning_index:
                            reasoning_part = reasoning_part.split(":", 1)[1].strip()
